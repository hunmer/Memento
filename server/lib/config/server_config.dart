...
class ServerConfig {
  ...
  factory ServerConfig.fromEnv() {
    return ServerConfig(
      ...
      tokenExpiryDays:
          int.tryParse(Platform.environment['TOKEN_EXPIRY_DAYS'] ?? '') ?? 7,
      enableCors:
  ...
  static String _generateDefaultSecret() {
    print('警告: 未设置 JWT_SECRET 环境变量，使用默认密钥（仅限开发环境）');
    return 'memento-dev-secret-key-${DateTime.now().millisecondsSinceEpoch}';
  }
