...
# -*- coding: utf-8 -*-
"""
批量替换 MaterialPageRoute 为 NavigationHelper 的脚本 v4.0
支持跨行、多行模式匹配
"""
...
def replace_material_page_route(content):
    """替换 MaterialPageRoute 调用 - 支持跨行"""

    # 模式1: navigator.push( 跨行的 MaterialPageRoute
    pattern1 = r"(\w+)\.push\(\s*context\s*,\s*\n\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)\n]+))?(?:,\s*maintainState:\s*([^,\)\n]+))?\s*\)"
    def repl1(match):
    ...
    # 模式2: Navigator.of(context).push( 跨行的 MaterialPageRoute
    pattern2 = r"Navigator\.of\(context\)\.push\(\s*\n\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)\n]+))?(?:,\s*maintainState:\s*([^,\)\n]+))?\s*\)"
    def repl2(match):
    ...
    # 模式3: Navigator.push( 跨行的 MaterialPageRoute
    pattern3 = r"Navigator\.push\(\s*\n\s*context\s*,\s*\n\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)\n]+))?(?:,\s*maintainState:\s*([^,\)\n]+))?\s*\)"
    def repl3(match):
        builder_code = match.group(1).strip()
        fullscreen = match.group(2).strip() if match.group(2) else None
        maintain = match.group(3).strip() if match.group(3) else None
    ...
    # 模式4: Navigator.of(context).pushReplacement( 跨行的 MaterialPageRoute
    pattern4 = r"Navigator\.of\(context\)\.pushReplacement\(\s*\n\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)\n]+))?\s*\)"
    def repl4(match):
        builder_code = match.group(1).strip()
        fullscreen = match.group(2).strip() if match.group(2) else None
    ...
    # 模式5: return MaterialPageRoute(...) - 在导航方法中返回路由 跨行
    pattern5 = r"return\s+\n\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)\n]+))?(?:,\s*maintainState:\s*([^,\)\n]+))?\s*;"
    def repl5(match):
        builder_code = match.group(1).strip()
        fullscreen = match.group(2).strip() if match.group(2) else None
...
def main():
    """主函数"""
    print("=" * 60)
    print("批量替换 MaterialPageRoute 为 NavigationHelper v4.0")
    print("=" * 60)
