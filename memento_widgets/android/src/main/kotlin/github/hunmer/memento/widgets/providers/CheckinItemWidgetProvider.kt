...
class CheckinItemWidgetProvider : BasePluginWidgetProvider() {
    ...
    override fun updateAppWidget(
        ...
        if (data != null) {
            val applied = setupCustomWidget(views, data)
            if (!applied) {
    ...
    private fun setupCustomWidget(views: RemoteViews, data: JSONObject): Boolean {
        return try {
            val stats = data.optJSONArray("stats")
            val checkinCount = stats?.optJSONObject(0)?.optString("value", "0") ?: "0"
            views.setTextViewText(R.id.widget_checkin_count, checkinCount)

            val weekState = stats?.optJSONObject(1)?.optString("value", "") ?: ""
            
            val checks = weekState
                .split(",")
                .map { it.trim() == "1" }
