...
class ClientCheckinRepository extends ICheckinRepository {
  ...
  ClientCheckinRepository({required this.storage, this.pluginId = 'checkin'});
  ...
  Future<Result<CheckinItemDto>> addCheckinRecord(
    ...
    try {
      ...
      final updatedRecords = Map<String, List<CheckinRecordDto>>.from(
        item.checkInRecords,
      );
  ...
  Future<Result<CheckinStatsDto>> getStats() async {
    try {
      ...
      final totalCheckins = items.fold<int>(0, (sum, item) {
        return sum +
            item.checkInRecords.values.fold<int>(
              0,
              (itemSum, records) => itemSum + records.length,
            );
      });
