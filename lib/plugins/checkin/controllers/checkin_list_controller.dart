...
class CheckinListController {
  ...
  List<Map<String, dynamic>> buildGroupListItems() {
    ...
    for (var group in groups) {
      ...
      final completedCount = groupItems.where((item) => item.isCheckedToday()).length;
      items.add({
        'group': group,
        'items': groupItems,
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  if (newGroupName.isNotEmpty && (!isEditing || newGroupName != group)) {
                    if (isEditing) {
                      // 更新所有该分组下的打卡项目
                    ...
                    ScaffoldMessenger.of(parentContext).showSnackBar(
                      SnackBar(content: Text(isEditing ? '已更新分组"$newGroupName"' : '已创建新分组"$newGroupName"')),
                    );
                  } else {
                    Navigator.pop(context);
                  }
  ...
  // 更新指定分组中项目的顺序
  Future<void> updateItemsOrder(String group, List<CheckinItem> newOrder) async {
