...
class _ActivityGridViewState extends State<ActivityGridView> {
  ...
  // 调整时间范围以避免与现有活动重叠
  // 返回调整后的时间范围
  ({DateTime start, DateTime end}) _adjustTimeToAvoidOverlap(
    DateTime start,
    DateTime end,
  ) {
  ...
  Color _generateDistinctColor(String tag) {
    ...
    Color bestColor =
        HSLColor.fromAHSL(1.0, baseHue, saturation, lightness).toColor();
    ...
    for (int i = 0; i < maxAttempts; i++) {
      ...
      Color candidateColor =
          HSLColor.fromAHSL(1.0, newHue, saturation, lightness).toColor();
  ...
  void _onGridDragEnd() {
    if (_isDragging && _selectionStart != null && _lastEnteredTime != null) {
      ...
      if (durationInMinutes >= 5 && adjustedEnd.isBefore(now)) {
                ...
                child: Row(
                  children: List.generate(12, (index) {
                    return Expanded(
                      child: Center(
                        child: Text(
                          '${(index * 5 + 5) % 60}'.padLeft(
                            2,
                            '0',
                          ), // 显示从05开始，但在创建活动时会向前调整5分钟
