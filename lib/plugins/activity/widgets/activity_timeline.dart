...
class ActivityTimeline extends StatelessWidget {
  ...
  // Use a consistent primary color as in the HTML example
  ...
  IconData _getActivityIcon(ActivityRecord activity) {
    ...
    if (title.contains('sleep') || title.contains('睡觉'))
      return Icons.bed_outlined;
    if (title.contains('eat') || title.contains('food') || title.contains('吃饭'))
      return Icons.restaurant_outlined;
    if (title.contains('sport') ||
        title.contains('run') ||
        title.contains('运动'))
      return Icons.fitness_center_outlined;
    if (title.contains('work') || title.contains('工作'))
      return Icons.work_outline;
    if (title.contains('study') || title.contains('学习'))
      return Icons.book_outlined;
    if (title.contains('game') || title.contains('游戏'))
      return Icons.games_outlined;
    if (title.contains('shower') || title.contains('洗澡'))
      return Icons.shower_outlined;
    if (title.contains('clean') || title.contains('打扫'))
      return Icons.cleaning_services_outlined;
    if (title.contains('read') ||
        title.contains('阅读') ||
        title.contains('bible'))
      return Icons.menu_book_outlined;
    if (title.contains('rise') ||
        title.contains('get up') ||
        title.contains('起床'))
      return Icons.alarm;
  ...
  Widget _buildTimelineItem(BuildContext context, ActivityRecord activity) {
    ...
    // Calculate height based on duration, with a minimum height.
    final double height = (duration.inMinutes * 1.0).clamp(64.0, 400.0);
          ...
          padding: const EdgeInsets.only(bottom: 8.0),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Center(
                  child:
                      (activity.mood != null && activity.mood!.isNotEmpty)
                          ? Text(
                            activity.mood!,
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        '${ActivityLocalizations.of(context).unrecordedTimeText} (${_formatDuration(start, end)}). ${ActivityLocalizations.of(context).tapToRecordText}',
                        style: TextStyle(
                          color: Theme.of(context).colorScheme.onSurfaceVariant,
                          fontSize: 13,
                        ),
  ...
  List<Widget> _buildTimelineSlices(BuildContext context) {
    ...
    // Use the date of the first activity as the reference for the start of the day
    final dayStart = DateTime(
      activities.first.startTime.year,
      activities.first.startTime.month,
      activities.first.startTime.day,
    );
    DateTime lastEventEnd = dayStart;

    // Gap from start of day to first activity
    ...
    for (int i = 0; i < activities.length; i++) {
      ...
      // Gap between previous activity and this one
    ...
    // Gap from last activity to now (if it's today)
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    if (lastEventEnd.year == today.year &&
        lastEventEnd.month == today.month &&
        lastEventEnd.day == today.day) {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        // The dashed timeline
        Positioned(
          top: 16, // Match listview vertical padding
          left:
