...
class ClientTrackerRepository implements ITrackerRepository {
  ...
  Future<Result<bool>> deleteGoal(String id) async {
    try {
      ...
      await _writeRecordsData({'records': filteredRecords.map((r) => r.toJson()).toList()});
  ...
  Future<Result<RecordDto>> addRecord(RecordDto record) async {
    try {
      ...
      final records = _parseRecordsList(recordsData);
      records.add(record);
      await _writeRecordsData({'records': records.map((r) => r.toJson()).toList()});
