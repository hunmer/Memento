...
class ChatPlugin extends BasePlugin {
  ...
  Future<void> initializeDefaultData() async {
    ...
    // 创建默认用户
    final user1 = User(id: '1', username: '我');
    final user2 = User(id: '2', username: '张三');
    final user3 = User(id: '3', username: '李四');
    final user4 = User(id: '4', username: '王五');
    ...
    final defaultChannels = [
      {
        'id': 'general',
        'title': '通用',
        'icon': '58136:MaterialIcons:null',
        'priority': 1,
        'members': [
          UserSerializer.toJson(user1),
          UserSerializer.toJson(user2),
          UserSerializer.toJson(user3),
        ],
      },
      {
        'id': 'team',
        'title': '团队',
        'icon': '58089:MaterialIcons:null',
        'priority': 0,
        'members': [
          UserSerializer.toJson(user1),
          UserSerializer.toJson(user3),
          UserSerializer.toJson(user4),
        ],
      },
    ];

    // 为每个频道创建默认消息
    for (var channelData in defaultChannels) {
      ...
      // 初始化频道信息
      await storage.initializeWithDefault('$channelPath/channel.json', {
        'channel': channelData,
      });
      ...
      await storage.initializeWithDefault('$channelPath/messages.json', {
