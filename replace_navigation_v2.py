...
# -*- coding: utf-8 -*-
"""
批量替换 MaterialPageRoute 为 NavigationHelper 的脚本 v2.0
支持更多复杂模式
"""
...
def replace_material_page_route(content):
    """替换 MaterialPageRoute 调用 - 改进版本"""

    # 模式1: Navigator.of(context).push(MaterialPageRoute(...))
    pattern1 = r"Navigator\.of\(context\)\.push\(\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)]+))?(?:,\s*maintainState:\s*([^,\)]+))?\)"
    def repl1(match):
    ...
    # 模式2: Navigator.push(context, MaterialPageRoute(...))
    pattern2 = r"Navigator\.push\(\s*context,\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)]+))?(?:,\s*maintainState:\s*([^,\)]+))?\)"
    def repl2(match):
        builder_code = match.group(1).strip()
        fullscreen = match.group(2).strip() if match.group(2) else None
        maintain = match.group(3).strip() if match.group(3) else None
    ...
    # 模式3: Navigator.of(context).pushReplacement(MaterialPageRoute(...))
    pattern3 = r"Navigator\.of\(context\)\.pushReplacement\(\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*(?:,\s*fullscreenDialog:\s*([^,\)]+))?\)"
    def repl3(match):
        builder_code = match.group(1).strip()
        fullscreen = match.group(2).strip() if match.group(2) else None
    ...
    # 模式4: Navigator.of(context).pushAndRemoveUntil(..., ...)
    pattern4 = r"Navigator\.of\(context\)\.pushAndRemoveUntil\(\s*MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)\s*,\s*(.*?)\)"
    def repl4(match):
        builder_code = match.group(1).strip()
    ...
    # 模式5: 简化版本 - 直接替换 MaterialPageRoute(...)
    pattern5 = r"MaterialPageRoute(?:<[^>]+>)?\(\s*builder:\s*\([^)]*\)\s*=>\s*(.*?)\s*\)"
    def repl5(match):
        builder_code = match.group(1).strip()
        return f"MaterialPageRoute(builder: (context) => {builder_code})"
...
def main():
    """主函数"""
    print("=" * 60)
    print("批量替换 MaterialPageRoute 为 NavigationHelper v2.0")
    print("=" * 60)
